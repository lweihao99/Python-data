...
def caricaDatiCapsule(fn):
    ...
    for line in file:
        ...
        i = 2
        while i < len(content):
            tipo_capsula = content[i]
            quantity = int(content[i+1])
            pz_uni = float(content[i+2])
            li.append((id_cliente, date, tipo_capsula, quantity, pz_uni))
            i += 3
    return li
...
def totaliVenditeCapsule(ds):
    ...
    pz_dict = {}
    for item in ds:
        (id_cliente, date, tipo_capsula, quantity, pz_uni) = item
        if tipo_capsula not in quantity_dict and tipo_capsula not in pz_dict:
            quantity_dict[tipo_capsula] = 0
            pz_dict[tipo_capsula] = 0
        quantity_dict[tipo_capsula] += quantity
        pz_dict[tipo_capsula] += quantity*pz_uni

    for key in quantity_dict:
        if key not in di:
            di[key] = [quantity_dict[key], pz_dict[key]]
