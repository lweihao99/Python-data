...
def calcolaBaricentri(filename1):
    ...
    for line in file:
        ...
        y = int(content[1])
        banda = content[2]
        nvolte = int(content[3])

        if banda not in di:
            coo_x[banda] = x * nvolte
            coo_y[banda] = y * nvolte
            di[banda] = nvolte
        else:
            coo_x[banda] += x * nvolte
            coo_y[banda] += y * nvolte
    ...
    diOut = {}
    for banda in di:
        baricX = int(coo_x[banda] / di[banda])
        baricY = int(coo_y[banda] / di[banda])
        diOut[banda] = (baricX, baricY)
...
#   deve contenere tante coppie chiave valore quante sono quelle presente nel
#   parametro dizbar passato in ingresso alla funzione. Per calcolare la distanza
#   tra due punti di coordinate (x1,y1) e (x2,y2) usate la distanza euclidea
...
def distanza(baricx, baricy, coordx, coordy):
    return (baricx-coordx)**2+(baricy-coordy)**2


def cityCoord(x, y, diz):
    tempMinDist = 1000000000
    tempCittaPiuVicina = ''
    for key in diz:
        value = diz[key]
        coordx = value[0]
        coordy = value[1]
        d = distanza(x, y, coordx, coordy)
        if d < tempMinDist:
            tempCittaPiuVicina = key
            tempMinDist = d
...
def cittaVicine(fileName2, dizbar):
    ...
    cityCoo = caricaCity(fileName2)
    dizOutput = {}
    for banda in dizbar:
        baricX = dizbar[banda][0]
        baricY = dizbar[banda][1]
        city = cityCoord(baricX, baricY, cityCoo)
        dizOutput[banda] = city
...
fn1 = 'Documents/Esame uni/2016-17/2017_02_02_trcrimine_sol/episodi.txt'
dib = calcolaBaricentri(fn1)
print('Risultato prima funzione: ')
print(dib)
fn2 = 'Documents/Esame uni/2016-17/2017_02_02_trcrimine_sol/citta.txt'
