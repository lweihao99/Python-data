...
######### Toglimi dal testo
def classeInquinante(targa):
    i=len(targa)-1
    while i>=0 and targa[i] not in cifre:
        i=i-1
    ultimap = targa[i+1:] # estraggo l'ultima parte
    ch2pos={}
...
def caricaAccessi(nomeFile):
    ...
    for line in f:
        ...
        line=line.strip('\r')
        records = line.split(',')
        (targa, data, ora) = records
        #targa_automobile,gg/mm/aa,hh:mm\r\n
        #fye7973ds,19/1/2016,2:31
        (gg,mese,aa)=data.split('/')
        gg=int(gg)
        mese=int(mese)
        aa=int(aa)
        (hh,mm)=ora.split(':')
        hh=int(hh)
        mm=int(mm)
        ci=classeInquinante(targa)
        tr.append( (targa, gg, mese, aa, hh, mm, ci) )
    return tr
...
######### Toglimi dal testo
def estraiUltimoNumeroTarga(targa):
    i=len(targa)-1
    while i>=0 and targa[i] not in cifre:
        i=i-1
    ultimost = targa[i]
...
def calcolaCostiAuto(dati):
    ...
    targa2importo={}
    for (targa, gg, mm, aa, hh, mm, ci) in dati:
        if targa not in targa2importo:
            targa2importo[targa]=0
        tassa=ci*1.5
        un = estraiUltimoNumeroTarga(targa)
        if mm==1 or mm==2:
            targapari=pari(estraiUltimoNumeroTarga(targa))
            ggpari=pari(gg)
            if targapari!=ggpari:
                tassa=50
        targa2importo[targa]+=tassa
    return targa2importo
...
######### Toglimi dal testo
'''
def writeli(fout, li, sep=',', newLine='\r\n'):
    li2=[str(el) for el in li]
    st=sep.join(li2)+newLine
    fout.write(st)
...
def creaDataset():
    fn='transiti.csv'
    fout = open(fn,'w')
    fout.write('Targa,data,ora\r\n')
    targhe=[]
    for i in xrange(50): # genero 50 targhe
        targa=""
        for k in xrange(random.randint(2,4)): # aggiungo le prime lettere dell'alfabeto
            targa+=alfabeto[random.randint(0,24)]
        for k in xrange(random.randint(2,4)): # aggiungo le cifre numeriche
            targa+=str(random.randint(0,9))
        for k in xrange(random.randint(2,4)): # aggiungo le ultime lettere dell'alfabeto
            targa+=alfabeto[random.randint(0,24)]
        targhe.append(targa)
    anno=2016
    for mese in xrange(1,7):
        if mese == 2:
            ggmese=28
            ...
            ggmese=31
        for gg in random.sample(range(1,ggmese+1),5):
            for hh in random.sample(range(0,24), 5):
                for mm in random.sample(range(0,60), 5):
                    for ti in random.sample(range(0, len(targhe)-1),5):
                        targa=targhe[ti]
                        #targa_automobile,gg/mm/aa,hh:mm\r\n
                        li = [targa, str(gg)+'/'+str(mese)+'/'+str(anno), str(hh)+':'+str(mm)]
                        writeli(fout, li)
    fout.close()
