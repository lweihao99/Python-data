...
def caricaDatiCarrelli(fn):
    ...
    for item in file:
        ...
        if operation == 'ADD':
            customer = int(content[1])
            id_prodotto = int(content[3])
            quantity = int(content[4])
            datalist.append((customer, operation, id_prodotto, quantity))
        ...
        if operation == 'CHECKOUT':
            customer = int(content[1])
            datalist.append((customer, operation))
...
def calcolaComposizione(ds):
    ...
    for item in ds:
        ...
        if customer not in carrelli:
            carrelli[customer] = {}

        if operation == 'ADD':  # prodotti aggiunti
            id_prodotto = item[2]
            qt = item[3]
            if id_prodotto not in carrelli[customer]:
                carrelli[customer][id_prodotto] = 0
            carrelli[customer][id_prodotto] += qt

        elif operation == 'DROP':  # prodotti eliminati
            id_prodotto = item[2]
            qt = item[3]
            carrelli[customer][id_prodotto] -= qt

        elif operation == 'CHECKOUT':  # prodotti acquistati
            temp_dic = {}
            for idProdotto in carrelli[customer]:
                qt = carrelli[customer][idProdotto]
                if qt != 0:
                    temp_dic[idProdotto] = qt
            li.append((customer, temp_dic))
            carrelli[customer] = {}
    return li
...
def combinazioneMaggiore(ds):
    ...
    for item in ds:
        customer = item[0]
        for id_prodotto in item[1]:
            quantity = item[1][id_prodotto]
    ...
    max_st = ""
    for key in dict_a:
        if dict_a[key] > max_qt:
            max_qt = dict_a[key]
            max_st = key

    return (max_st, max_qt)
