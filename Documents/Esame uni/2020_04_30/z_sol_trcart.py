...
def caricaDatiCarrelli(fn):
    ...
    pass
    fd = open(fn, 'r')
    ds = []  #  struttura dati da restituire
    #    [ (id_cliente1, 'ADD', id_prodottoA, qtA),
    #          (id_cliente2, 'DROP', id_prodottoB, qtB),
    #          (id_cliente3, 'CHECKOUT'),
    fd.readline()  #  salto l'intestazione
    for line in fd:
        line = line.strip('\n').strip('\r')
        fields = line.split(';')
        # TIME;CUSTOMER;OPERATION;...;...
        ...
        idCli = int(fields[1])
        tipoOp = fields[2]
        if tipoOp == 'ADD' or tipoOp == 'DROP':
            ds.append((idCli, tipoOp, int(fields[3]), int(fields[4])))
        elif tipoOp == 'CHECKOUT':
            ds.append((idCli, tipoOp))
    return ds
...
def calcolaComposizione(ds):
    ...
    # Implementa il codice della funzione qua sotto. Questa riga può essere cancellata.
    pass
    carrelli = {}  # dizionario usato per gestire l'inserimento di valori nel carrello
    tr = []  # struttura dati da restituire
    for record in ds:
        # print('*')
        idCliente = record[0]
        tipoOp = record[1]
        if idCliente not in carrelli:
            carrelli[idCliente] = {}
        if tipoOp == 'ADD':
            # (id_cliente1, 'ADD', id_prodottoA, qtA),
            idProd = record[2]
            qt = record[3]
            if idProd not in carrelli[idCliente]:
                carrelli[idCliente][idProd] = 0
            carrelli[idCliente][idProd] += qt
        elif tipoOp == 'DROP':
            idProd = record[2]
            qt = record[3]
            carrelli[idCliente][idProd] -= qt  # non serve fare i controlli
        elif tipoOp == 'CHECKOUT':
            # carico i valori in ds e tolgo i dati del cliente da carrelli
            dic = {}
            for idProd in carrelli[idCliente]:
                qt = carrelli[idCliente][idProd]
                if qt != 0:
                    dic[idProd] = qt
            tr.append((idCliente, dic))
            carrelli[idCliente] = {}  #  svuoto il carrello del cliente
    return tr
...
#   Per esempio ('18*23':55) significa che l'id_cliente 18 ha acquistato l'id_prodotto 23
...
def combinazioneMaggiore(ds):
    ...
    for el in ds:
        # (id_cliente1, {id_prodottoA:qtA, id_prodottoB:qtB, ...})
        idCliente = el[0]
        for idProdotto in el[1]:
            qt = el[1][idProdotto]
            st = str(idCliente)+'*'+str(idProdotto)
            if st not in di:
                di[st] = 0
            di[st] += qt
    maxQt = -1
...
fnv = 'Esame-uni/Documents/Esame uni/2020_04_30/cart.csv'
