...
def leggiPiste(filePiste):
    ...
    for line in f:
        ...
        idPista=int(records[0])
        nomePista=records[1]
        l=int(records[2])
        lungArrot=arrotondaDistanze(l)
        ds[idPista] = (nomePista,lungArrot)
    return ds
...
def leggiPassaggi(nomeFilePassaggi):
    ...
    for line in f:
        ...
        records=line.split(sep)
        idCli=int(records[0])
        giorno=int(records[1])
        hm = records[2].split(':')
        hh = int(hm[0])
        mm = int(hm[1])
        id_pista=int(records[3])
        punti=puntiBonusPiste(giorno, hh, mm)
        ds.append( (idCli, id_pista, punti) )
    return ds
