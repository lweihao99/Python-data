...
# deve riuscire ad individuare tutte le navi della flotta nel minor
# tempo possibile.
# L'associazione vuole assegnare un punteggio ad ogni partita svolta,
# per poter valutare le performance di un giocatore.
# SEGUE ORA UNA BREVE DESCRIZIONE DEL GIOCO (per chi non conoscesse come si
# svolge il gioco).
...
# contigue, cioe' una vicino all'altra.
...
# Per semplicita', puo' accadere che due navi diverse si ritrovino attaccate l'una
# all'altra.
...
def caricaTiri(fname):
    ...
    for line in f:
        ...
        lettera = line[0] # la lettera e' sempre il primo carattere
        xStr = line[1:] # il valore numerico sono i seguenti
        x=int(xStr)
        y = lettera2num(lettera)
        li.append( (y,x) )
    f.close()
    return li
...
def caricaDisposizione(fname):
    pass  # Implementa il codice della funzione qua sotto. Questa riga puo' essere cancellata.
    scacchiera=restituisciScacchieraVuota()
    f=open(fname,'r')
    f.readline() # salto la riga con la descrizione del tipo di navi impiegate
    for line in f:
        line = line.strip('\r\n')
        elems = line.split(';')
        for el in elems:
...
# creo questa funzione accessoria
def colpito(row, column, chessboard):
...
def calcolaStatisticheTiri(tiri, scac):
    ...
    numColpiti=0
    numInsuccessi=0
    for (row, column) in tiri:
        if colpito(row, column, scac):
            numColpiti+=1
        else:
            numInsuccessi+=1
        di[numeroTiro]=(numColpiti,numInsuccessi)
        numeroTiro+=1
    return di
...
def controlloFlotta(disposizioniFileName):
    fdis=open(disposizioniFileName,'r')
    #carico i requisiti
    reqLine = fdis.readline()
    reqLi = reqLine.split(';')
    i=0
    reqDiz = {}
    while i < len(reqLi):
        reqDiz[i+1] = int(reqLi[i])
        i+=1

    risultatoControlli=True

    # ora controllo le navi
    for line in fdis:
        line = line.strip('\r\n')
        tipoNave = len(line.split(';'))
        if tipoNave not in reqDiz:
            risultatoControlli=False # abbiamo trovato una nave con dimensioni diverse da quelle richieste
        else:
            reqDiz[tipoNave] -= 1
    #verifico che reqDiz abbia tutte chiavi con valori a 0
    for k in reqDiz:
        if reqDiz[k]!=0:
            risultatoControlli=False
    fdis.close()
    return risultatoControlli
...
def calcolaPunti(tiri, scac):
    ...
    for (row, column) in tiri:
        if colpito(row, column, scac):
            punti+=successiPrecedenti
            successiPrecedenti+=1
        else:
            successiPrecedenti=0
    return punti
