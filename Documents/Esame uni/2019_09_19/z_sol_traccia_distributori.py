...
def caricaDatiMacchinette(fn):
    ...
    pass
    li = []
    fd = open(fn, 'r')  # 以只读模式打开文件
    fd.readline()  #  salto l'intestazione,逐行读取

    for line in fd:
        line = line.strip('\n')
        line = line.strip('\r')
        record = line.split(';')
        idCliente = int(record[0])
        ii = 1
        while ii < len(record):
            operazione = int(record[ii])
            importo = int(record[ii+1])
            li.append((idCliente, operazione, importo))
            ii += 2
    return li
...
def incassoTotale(ds):
    ...
    pass
    incasso = 0
    for (idCliente, operazione, importo) in ds:
        if importo > 150 and importo < 200:
            incasso += importo
    return incasso
...
def saldoResiduo(ds):
    ...
    saldo = {}
    for (idCliente, operazione, importo) in ds:
        if idCliente not in saldo:
            saldo[idCliente] = 0
        if operazione == 0:
            saldo[idCliente] += importo  # ricarica
        else:
            saldo[idCliente] -= importo  # spesa
    return saldo
...
def prodottoPiuVenduto(ds):
    ...
    cli2acquisti = {}
    for (idCliente, operazione, importo) in ds:
        if idCliente not in cli2acquisti:
            cli2acquisti[idCliente] = {}
        if operazione != 0:
            if operazione not in cli2acquisti[idCliente]:
                cli2acquisti[idCliente][operazione] = 1
            else:
                cli2acquisti[idCliente][operazione] += 1
    cli2maxAqu = {}
    for idCliente in cli2acquisti:
        # inserisco almeno l'informazione base
        cli2maxAqu[idCliente] = [-1, -1]
        for idProdotto in cli2acquisti[idCliente]:
            nacq = cli2acquisti[idCliente][idProdotto]
            if nacq > cli2maxAqu[idCliente][1]:
                cli2maxAqu[idCliente][0] = idProdotto
                cli2maxAqu[idCliente][1] = nacq
    return cli2maxAqu
