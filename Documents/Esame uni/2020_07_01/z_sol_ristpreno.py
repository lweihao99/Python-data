...
def caricaDatiPrenotazioni(fn):
    ...
    for line in fd:
        ...
        fields = line.split(';')
        id_ristorante = int(fields[0])
        giorno, oraInizio, durata = fields[1].split(':')
        giorno = int(giorno)
        oraInizio = int(oraInizio)
        durata=int(durata)
        idCliLi=[]
        for ind in range(2, len(fields)):
            idCliLi.append(int(fields[ind]))
        li.append( (id_ristorante, giorno, oraInizio, durata, idCliLi)  )
    fd.close()
    return li
...
def numeroPrenotazioni(ds):
    ...
    for pren in ds:
        ...
        oraInizio=pren[2]
        durata=pren[3]
        for ora in range(oraInizio, oraInizio+durata):
            k=str(giorno)+'_'+str(ora)
            if k in di[idRistorante]:
                di[idRistorante][k] += 1
            else:
                di[idRistorante][k] = 1
        # in altelrnativa al ciclo for qua sopra
        # ora=oraInizio
        # while ora<oraInizio+durata:
        #    ...
        #    ora+=1
    return di
...
def coppieFrequenti(ds):
    ...
    for pren in ds:
        ...
        while ii<len(liCli):
            jj=ii+1
            while jj<len(liCli): # se len(liCli)==1, non entra nel ciclo, quindi le prenotazioni dei singoli non vengono contate 
                if liCli[ii] < liCli[jj]:
                    # in questo modo, l'id minore e' sempre a sinistra
                    k = str(liCli[ii]) + '_' + str(liCli[jj]) # si poteva utilizzare anche k=(liCli[ii], liCli[jj])
                else:
                    k = str(liCli[jj]) + '_' + str(liCli[ii])
                if k in di:
                    di[k] += 1
                else:
                    di[k] = 1
    ...
    for k in di:
        if di[k] > maxCount:
            maxCount=di[k]
            coppiaMax=k
    (id1, id2) = coppiaMax.split('_')
    return (int(id1), int(id2), maxCount)
