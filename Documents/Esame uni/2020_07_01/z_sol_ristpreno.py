...
def caricaDatiPrenotazioni(fn):
    ...
    for line in fd:
        # Come terminatore di riga e' usato \r\n. Levo prima il \n e poi il \r
        line = line.strip('\n').strip('\r')
        fields = line.split(';')
...
def coppieFrequenti(ds):
    ...
    for pren in ds:
        ...
        while ii < len(liCli):
            jj = ii+1
            # se len(liCli)==1, non entra nel ciclo, quindi le prenotazioni dei singoli non vengono contate
            while jj < len(liCli):
                if liCli[ii] < liCli[jj]:
                    # in questo modo, l'id minore e' sempre a sinistra
                    # si poteva utilizzare anche k=(liCli[ii], liCli[jj])
                    k = str(liCli[ii]) + '_' + str(liCli[jj])
                else:
