...
def caricaDatiCoworking(fn):
    ...
    for item in file:
        ...
        id_stanza = int(new_content[-1])
        giorno = new_content[-2]
        temp_li = [id_stanza, giorno]
        for idUtente in new_content[:-2]:
            temp_li.append(int(idUtente))
        datalist.append(temp_li)
    return datalist
...
def costiUtilizzo(ds):
    ...
    for item in ds:
        id_stanza = item[0]
        for id_utente in item[2:]:
            if id_utente not in sol:
                sol[id_utente] = 0
            if id_stanza == 0:  # open space
                sol[id_utente] += 2
            else:
                num_persone = len(item)-2
                sol[id_utente] += 60/num_persone
    return sol
...
def macroGruppi(ds):
    ...
    for item in ds:
        ...
        for k in item[2:]:
            id_macro = int(k) % 20
            if id_stanza >= 1 and id_macro not in temp_dict:
                temp_dict[id_macro] = 1
            elif id_stanza >= 1 and id_macro in temp_dict:
                temp_dict[id_macro] += 1
    return temp_dict
...
def composizioneTeam(ds):
    ...
    for item in ds:
        ...
        if id_stanza > 0:
            idTeamLeader = 0
            eleTeam = []
            for idUtente in item[2:]:
                if idUtente < 20:
                    idTeamLeader = idUtente
                else:
                    eleTeam.append(idUtente)
            if idTeamLeader not in temp_dict:
                temp_dict[idTeamLeader] = eleTeam
            else:
                for el in eleTeam:
                    if el not in temp_dict[idTeamLeader]:
                        temp_dict[idTeamLeader].append(el)
