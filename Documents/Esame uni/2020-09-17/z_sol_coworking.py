...
def caricaDatiCoworking(fn):
    ...
    for line in fd:
        line = line.strip('\n').strip('\r')
        record = line.split(';')
        idStanza = int(record[-1])
        data = record[-2]
        liInt = [idStanza, data]
        # ciclo su tutti gli elementi tranne gli ultimi 2
        for idUtente in record[:-2]:
            liInt.append(int(idUtente))
        liExt.append(liInt)
    return liExt
...
def costiUtilizzo(ds):
    ...
    for record in ds:
        idStanza = record[0]
        for idUtente in record[2:]:  # ciclo dal secondo elemento in poi
            if idUtente not in diCosti:
                diCosti[idUtente] = 0
            if idStanza == 0:
                diCosti[idUtente] += 2
            else:
                nPersone = len(record)-2
                diCosti[idUtente] += 60.0 / nPersone
    return diCosti
...
def macroGruppi(ds):
    ...
    for record in ds:
        for idUtente in record[2:]:  # ciclo dal secondo elemento in poi
            macroGruppo = idUtente % 20
            if macroGruppo not in diUtilizzo:
                diUtilizzo[macroGruppo] = 1
            else:
                diUtilizzo[macroGruppo] += 1
    return diUtilizzo
...
def composizioneTeam(ds):
    ...
    for record in ds:
        idStanza = record[0]
        if idStanza > 0:  # altrimenti e' nell'open space, dove sono presenti membri di team diversi
            teamLeader = -1
            elementiTeam = []
            # Cerco il teamLeader e gli elementi del team
            for idUtente in record[2:]:  # ciclo dal secondo elemento in poi
                # individuo il team teader e gli elementi del team
                if idUtente < 20:
                    teamLeader = idUtente
                else:
                    elementiTeam.append(idUtente)
            # ora inserisco le informazioni in diTeam
            if teamLeader not in diTeam:
                diTeam[teamLeader] = elementiTeam
            else:
                for el in elementiTeam:
                    # inserisco i membri del team solo se sono nuovi
                    if el not in diTeam[teamLeader]:
                        diTeam[teamLeader].append(el)
    return diTeam
