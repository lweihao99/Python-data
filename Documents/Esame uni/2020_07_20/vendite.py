...
def calcolaDurataAnnuncio(ds):
    ...
    for tu in ds:
        ...
        if type_ == 'pubblicazione':
            data = tu[1]
            id_annuncio = tu[2]
            dict_a[id_annuncio] = giorno_data(data)
        elif type_ == 'rimozione':
            data = tu[1]
            id_annuncio = tu[2]
...
def prezziCategorieAbitazioni(ds):
    # Implementa il codice della funzione qua sotto. La riga con il pass puo' essere cancellata.
    dict_a = {'0-100': 0, '101-200': 0, '201_e_superiore': 0}
    dict_b = {'0-100': 0, '101-200': 0, '201_e_superiore': 0}
    dict_sol = {}
    for item in ds:
        type_ = item[0]
        if type_ == 'catasto':
            ...
            if 0 <= m2 <= 100:
                prezzo_m2 = item[5]
                dict_a['0-100'] += m2*prezzo_m2
                dict_b['0-100'] += m2

            elif 101 <= m2 <= 200:
                prezzo_m2 = item[5]
                dict_a['101-200'] += m2*prezzo_m2
                dict_b['101-200'] += m2

            elif m2 >= 201:
                prezzo_m2 = item[5]
                dict_a['201_e_superiore'] += m2*prezzo_m2
                dict_b['201_e_superiore'] += m2
    for key in dict_a:
        dict_sol[key] = dict_a[key] / dict_b[key]
    return dict_sol
