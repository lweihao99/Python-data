...
def caricaDatiAbitazioni(fn):
    ...
    li=[]
    for ii in range(3):
        fd.readline() # leggo senza memorizzare le 3 righe dell'intestazione
    for line in fd:
        line=line.strip('\r\n')
        fields = line.split(';')
        tipo = fields[1]
        if tipo=='pubblicazione':
            # Data;tipoOperazione(pubblicazione);idAnnuncio;idAbitazione;M2;prezzoM2Proposto
            # 2010_06_12;pubblicazione;1438;DONN;100;9961
            data = fields[0]
            idAnnuncio=int(fields[2])
            idAbitazione=fields[3]
            m2=int(fields[4])
            prezzoM2Proposto=int(fields[5])
            li.append( (tipo, data, idAnnuncio, idAbitazione, m2, prezzoM2Proposto) )
        elif tipo=='rimozione':
            #Data;tipoOperazione(rimozione);idAnnuncio
            data = fields[0]
            idAnnuncio=int(fields[2])
            li.append( (tipo, data, idAnnuncio) )
        elif tipo=='catasto':
            #Data;tipoOperazione(catasto);idProprietario;idAbitazione;M2;prezzoM2Vendita
            data = fields[0]
            idProprietario=int(fields[2])
            idAbitazione=fields[3]
            m2=int(fields[4])
            prezzoM2Vendita=int(fields[5])
            li.append( (tipo, data, idProprietario, idAbitazione, m2, prezzoM2Vendita) )
    return li
...
# se vi puo' far comodo definire delle funzioni accessorie potete farlo qua sotto
def giornoAnno(dataPar): # '2010_07_12'
    # funzione di servizio, creata per processare le date
    fields = dataPar.split('_')
    gg = int(fields[2])
    mm = int(fields[1]) 
    #aa = int(fields[0])
    ggMesePrec = [0] * 12 # [0, 0, ..., 0]
    ii=1 # si parte da febbraio
    prog=0
    # Calcolo per ogni mese i giorni trascorsi dall'inizio dell'anno alla fine del mese precedente es., [0, 31, 31+28, ...]
    for mese in ['gennaio', 'febbraio', 'marzo', 'aprile', 'maggio', 'giugno', 'luglio', 'agosto', 'settembre', 'ottobre', 'novembre']: # dicembre non serve
        prog+=dizGiorniMese[mese]
        ggMesePrec[ii]=prog
        ii+=1
    return gg+ggMesePrec[mm-1] # i mesi sono numerati da 1 a 12, gli indici della lista da 0 a 11

# altra possibilita':
def giornoAnno2(dataPar): # '2010_07_12'
    dizgm2 = {1: 31, 2: 28, 3: 31, 4: 30,
                 5: 31, 6: 30, 7: 31, 8: 31,
...
def calcolaDurataAnnuncio(ds):
    ...
    diG={}    
    for el in ds:
        # pubblicazione li.append( (tipo, data, idAnnuncio, idAbitazione, m2, prezzoM2Proposto) )
        # rimozione li.append( (tipo, data, idAnnuncio) )
        tipo=el[0]
        if tipo=='pubblicazione':
            data=el[1]
            idAnnuncio = el[2]
            # Inserisco una struttura dati
            diPub[idAnnuncio]=giornoAnno(data)
        elif tipo=='rimozione':
            data=el[1]
            idAnnuncio = el[2]
            diRim[idAnnuncio]=giornoAnno(data) # a questo punto si sarebbe potuto calcolare direttamente diG ed evitare il ciclo for successivo
    for idAnnuncio in diPub:
        diG[idAnnuncio] = diRim[idAnnuncio] - diPub[idAnnuncio]
    return diG

# altra possibile soluzione
def calcolaDurataAnnuncio2(ds):
    # Implementa il codice della funzione qua sotto. La riga con il pass puo' essere cancellata.
    pass
    di={}
    for el in ds:
...
def prezziCategorieAbitazioni(ds):
    ...
    cat1='0_100'
    cat2='101_200'
    cat3='201_e_superiori'
    diNum={cat1:0, cat2:0, cat3:0}
    diDen={cat1:0, cat2:0, cat3:0}
    diMed={}
    for el in ds:
        tipo=el[0]
        if tipo=='catasto':
            ...
            if m2<=100:
                ...
                cat=cat3
            diNum[cat] += m2*prezzoM2 # il costo totale di acquisto 
            diDen[cat] += m2
    for k in diNum:
        diMed[k] = float(diNum[k]) / diDen[k] # il float() sarebbe necessario solo in python2
    return diMed
